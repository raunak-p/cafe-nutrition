...
source_data <- read.csv("../Data/pp_data_oct13.csv", stringsAsFactors = FALSE)

# Attaching names to the Dataframe
col_names <- c(
...
colnames(source_data) <- col_names
...
source_data$first_name <- capitalize(tolower(source_data$first_name))
...
for (name in source_data$first_name){
  ...
  render(input = "Nutrition_Report_RMarkdown.rmd", output_file = paste0(iterator, "_", name, "_", capitalize(tolower(last_name))," ", "report", ".pdf"))
  iterator=iterator+1
}