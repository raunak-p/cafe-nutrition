...
source_data <- read.csv("../Data/cns_pp_actual_3.csv", stringsAsFactors = FALSE)
...
col_names <- c(
  ...
  "vegetables", "fruits",  "whole_grains",
...
#Making a new dataframe and attaching names
cleaned_source_data <- source_data

colnames(cleaned_source_data) <- col_names

#Attaching gender pronouns

cleaned_source_data <- mutate(cleaned_source_data,
    his_her = case_when(
    gender == "Male" ~ "his",
    gender == "Female" ~ "her"
  ),
...
#Calculating Points and adding Columns
...
#generating reports
for (name in cleaned_source_data$full_name){
  subgroup <- cleaned_source_data[cleaned_source_data$full_name == name,]
  render(input = "CNS_Preprimary_Report.rmd",output_file = paste0(name, "_",  'report', '.pdf'))
}